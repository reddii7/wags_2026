-- NULL play_order was sorted as "latest" (coalesce to max int), causing Home on week 7
-- and duplicate WK8 when row_number also assigned 8 to that row.

update public.rounds
set play_order = (regexp_match(trim(name), '^Week\s+(\d+)', 'i'))[1]::int
where play_order is null
  and trim(name) ~* '^Week\s+\d+$';

-- Re-apply home + results functions with nulls-last ordering (not max-int for null).

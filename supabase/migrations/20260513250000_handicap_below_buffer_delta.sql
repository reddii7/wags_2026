-- Below-buffer handicap bump was hardcoded (+0.1); drive it from handicap_rules per band.

alter table public.handicap_rules
  add column if not exists below_buffer_delta numeric(6, 2) not null default 0.1;

comment on column public.handicap_rules.below_buffer_delta is
  'Handicap index change when net stableford S is strictly below buffer_zone (typically +0.1).';

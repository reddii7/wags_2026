-- Align campaigns.kind with core schema campaigns.track (main_summer) so home RPC
-- and views stay consistent. Does not drop or rewrite unrelated data.

begin;

alter table public.campaigns add column if not exists kind text;
alter table public.campaigns add column if not exists year integer;
...
begin
  if exists (
    ...
    where col.table_schema = 'public'
      ...
      and col.column_name = 'track'
  ) then
    update public.campaigns
    set kind = 'summer_main'
    where kind is null
      and track::text = 'main_summer';

    update public.campaigns
    set kind = 'winter_reduced'
    where kind is null
      and track::text = 'winter';
  end if;
end $$;
...
begin
  ...
  if v_has_track then
    select
      case when c.kind is null then null else c.kind::text end,
      c.track::text
    into v_kind, v_track
    from public.campaigns c
    where c.id = p_campaign_id;
  else
    select case when c.kind is null then null else c.kind::text end
    into v_kind
    from public.campaigns c
    where c.id = p_campaign_id;
    v_track := null;
  ...
  v_is_summer :=
    nullif(trim(coalesce(v_kind, '')), '') = 'summer_main'
    or (
      (v_kind is null or trim(coalesce(v_kind, '')) = '')
      and nullif(trim(coalesce(v_track, '')), '') = 'main_summer'
    );
...
comment on function public.get_member_home_snapshot(uuid) is
  'Contract-v1 home slice. Summer = kind summer_main OR track main_summer; '
  'latest finalized summer_weekly; hero/stats; integer handicap steps; Best14 top 3 + ties; tier leaders.';

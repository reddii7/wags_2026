-- RS Cup, finals, away days must NOT adjust handicaps (§3.0 / §7).
-- finalize_round previously ran the handicap engine for every round_type.
-- This migration replaces the function with a guard: only summer_weekly and
-- winter_weekly rounds run the handicap loop.
...
begin
  ...
  -- Only summer_weekly rounds affect handicaps (§3.0 / §3.4 / §7).
  -- Winter, RS Cup, finals, and away days never run the handicap engine.
  ...
  -- Handicap engine: only for weekly rounds (§3.0).
...
comment on function public.finalize_round(uuid) is
  ...
  'members.handicap_index + member_handicap_state. Handicap engine runs only for summer_weekly '
  'rounds only (§3.0). Winter, RS Cup, finals, away_day: money settled, no handicap change.';

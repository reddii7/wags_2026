-- Summer weekly prize eligibility: sole top scorer needs 3 prior completed
-- summer_weekly rounds (any campaign/year). Ineligible sole tops roll the pot
-- (same money outcome as a tie). Does not apply to winter. Second place never wins.

begin;

alter table public.weekly_prize_state
  add column if not exists no_winner_reason text;

comment on column public.weekly_prize_state.no_winner_reason is
  'Null when there is a winner. Otherwise tie | min_prior_rounds (summer only).';
...
declare
  ...
  v_max_pts          integer := -1;
  v_tie              boolean := false;
  v_no_winner_reason text;
  v_prior_completions integer := 0;
...
begin
  ...
  v_winner_id := null;
  v_no_winner_reason := null;

  if v_tie then
    v_no_winner_reason := 'tie';
  elsif v_max_pts is not null then
    ...
    -- Summer weekly only: need 3 prior finalized summer_weekly completions (any year).
    -- Do not fall through to second place — treat like a rollover / tie.
    if v_round.round_type = 'summer_weekly' and v_winner_id is not null then
      select count(*)::integer
      into v_prior_completions
      from public.round_players rp
      join public.rounds r on r.id = rp.round_id
      where rp.member_id = v_winner_id
        and r.id <> p_round_id
      ...
      if coalesce(v_prior_completions, 0) < 3 then
        v_winner_id := null;
        v_no_winner_reason := 'min_prior_rounds';
      end if;
    end if;
  end if;

  v_current_week_seq := coalesce(
    nullif(v_round.play_order, 0),
    (regexp_match(trim(coalesce(v_round.name, '')), '^Week\s+(\d+)', 'i'))[1]::int
  );

  if v_current_week_seq is not null then
    ...
    where r.campaign_id = v_round.campaign_id
      ...
      and coalesce(
        nullif(r.play_order, 0),
        (regexp_match(trim(coalesce(r.name, '')), '^Week\s+(\d+)', 'i'))[1]::int
      ) < v_current_week_seq
    order by coalesce(
      nullif(r.play_order, 0),
      (regexp_match(trim(coalesce(r.name, '')), '^Week\s+(\d+)', 'i'))[1]::int
    ...
    where r.campaign_id = v_round.campaign_id
      ...
      and wps.superseded_at is null
      and (
        r.round_date::date < v_round.round_date::date
        or (r.round_date::date = v_round.round_date::date and r.id < v_round.id)
      )
  ...
  insert into weekly_prize_state
    (round_id, winner_member_id, paid_out_pence, to_bank_pence,
     rollover_carried_in, rollover_carried_out, bank_wallet, no_winner_reason)
  values
    (p_round_id, v_winner_id, v_paid_out, v_bank_pence,
     v_rollover_in, v_rollover_out, v_money.bank_wallet, v_no_winner_reason);
  ...
  return jsonb_build_object(
    ...
    'no_winner_reason',  v_no_winner_reason,
    'prior_completions', v_prior_completions,
...
comment on function public.finalize_round(uuid) is
  'Settle a round: prize money → weekly_prize_state; handicap adjustment for summer_weekly only. '
  'Rollover predecessor uses Play # or Week NN name when Play # was missing at finalize time. '
  'Summer weekly: sole top scorer needs >=3 prior finalized summer_weekly completions (any campaign) '
  'or pot rolls (no second-place fallback; no Champions qualification).';
...
begin
  ...
  order by r.play_order desc nulls last, r.round_date desc nulls last, r.id desc
  limit 1;
  ...
  select
    ...
    wps.no_winner_reason
  into v_paid_out_pence, v_roll_out_pence, v_winner_member_id, v_no_winner_reason
  ...
  if v_entrants = 0 then
    ...
    v_no_results := false;
  elsif v_no_winner_reason = 'min_prior_rounds' then
    v_hero :=
      v_names_joined
      || ' scored '
      || v_score_line
      || ' but needs 3 prior completed rounds to win, £'
      || trim(to_char(round(coalesce(v_roll_out_pence, 0)::numeric / 100.0, 2), 'FM999999990.00'))
      || ' carries to next week.';
    v_no_results := false;
...
comment on function public.get_member_home_snapshot(uuid) is
  'Contract-v1 home slice: latest scored summer_weekly (open or finalized), hero copy, stats, '
  'handicap integer steps only, Best14 rank<=3 with ties, division leaders tiers 1–4; min_prior_rounds rollover copy when sole top scorer is prize-ineligible.';
...
begin
  ...
  loop
    ...
    select
      ...
      wps.winner_member_id,
      wps.no_winner_reason
    into v_paid_out, v_roll_out, v_winner_id, v_no_winner_reason
    from public.weekly_prize_state wps
    ...
    if v_entrants = 0 then
      ...
      v_hero :=
        ...
        || trim(to_char(round(coalesce(v_roll_out, 0)::numeric / 100.0, 2), 'FM999999990.00'))
        || ' carries to next week.';
      v_winner_type := 'rollover_min_rounds';
      v_winner_names := to_jsonb(v_top_names);
...
comment on function public.get_member_results_contract(uuid) is
  'Contract-v1 results_view: summer_weekly rounds only; week_number from play_order when set.';

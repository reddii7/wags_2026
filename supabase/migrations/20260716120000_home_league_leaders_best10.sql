-- Regression fix: 20260715120000 rewrote get_member_home_snapshot and put
-- home league_leaders back on best_14_total. Rank/score by best_10_total again.
  ...
  if v_kind = 'summer_main' then
    ...
    from (
      select distinct on (la.tier)
        ...
        m.full_name,
        la.tier as position,
        v.best_10_total::numeric as total_score
      ...
      order by la.tier, v.best_10_total desc nulls last, m.id
    ) u;
  else
    v_b14 := '[]'::jsonb;
    v_ll := '[]'::jsonb;
...
comment on function public.get_member_home_snapshot(uuid) is
  ...
  'handicap integer steps only, Best14 rank<=3 with ties, division leaders tiers 1–4 by best 10; min_prior_rounds rollover copy when sole top scorer is prize-ineligible.';

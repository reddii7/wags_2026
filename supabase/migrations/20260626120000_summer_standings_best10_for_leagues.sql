-- Leagues rank by each member's best 10 summer weekly scores; Best 14 uses top 14.
-- v_summer_standings previously exposed only best_14_total, so leagues matched Best 14.
...
select
  ...
  m.full_name,
  la.tier,
  coalesce(sum(rp14.stableford_points), 0)::numeric as best_14_total,
  coalesce(sum(rp10.stableford_points), 0)::numeric as best_10_total
...
-- Home league leaders: top player per division by best 10 (not best 14).
...
comment on function public.get_member_home_snapshot(uuid) is
  ...
  'handicap integer steps only, Best14 rank<=3 with ties, division leaders tiers 1–4 (best 10).';

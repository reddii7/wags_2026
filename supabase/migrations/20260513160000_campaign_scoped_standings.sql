-- Standings & bank: one row set per campaign (Summer 2026 vs 2027, etc.)
-- Summer lateral must filter rounds by la.campaign_id (not all summer_weekly globally).

drop view if exists public.v_summer_standings cascade;
create view public.v_summer_standings as
select
  la.campaign_id,
...
-- Winter: members who appear in finalized winter_weekly rounds for that campaign (no players-table dependency).
drop view if exists public.v_winter_standings cascade;
create view public.v_winter_standings as
select
  c.id as campaign_id,
  c.label as campaign_label,
  c.year as campaign_year,
...
-- Bank: one balance row per campaign.
drop view if exists public.v_bank_dashboard cascade;
create view public.v_bank_dashboard as
select
  r.campaign_id,
  c.label as campaign_label,
  c.year as campaign_year,

#!/usr/bin/env python3
"""
Import 2025 summer weekly rounds into the greenfield schema (rounds + round_players).

- Source: legacy Supabase (same reads as import_2025_one_week.py).
- Target: campaign year=2025, kind=summer_main. Round names from legacy (e.g. Week 01 … Week 29).
- Does NOT call finalize_round (scores only). Finalize weeks in play order in admin when ready.

Single week:
  python3 supabase/scripts/import_2025_week_greenfield.py --week 1 \\
    -o scripts/sql/generated/import_week_01.sql

Batch (scores only; one BEGIN/COMMIT per week):
  python3 supabase/scripts/import_2025_week_greenfield.py --from-week 2 --to-week 29 \\
    -o scripts/sql/generated/import_weeks_02_29.sql
...
def fetch_old_competitions_2025() -> list[dict[str, Any]]:
...
def fetch_old_round_scores() -> list[dict[str, Any]]:
...
def build_sql(
    ...
    *,
    ...
    week_label = raw_name if raw_name else f"Week {week_no:02d}"
    comp_date = str(comp.get("competition_date") or "2025-01-01")[:10]
    ...
    return f"""-- Week {week_no}: {week_label} — scores only (not finalized).
...
with camp as (
  ...
  where year = 2025 and kind = 'summer_main'::public.campaign_kind
  limit 1
),
ins as (
...
def main() -> None:
    ...
    if range_mode:
        ...
        out = args.output or (
            f"scripts/sql/generated/import_weeks_{args.from_week:02d}_{args.to_week:02d}.sql"
        )
    else:
        ...
        out = args.output or f"scripts/sql/generated/import_week_{args.week:02d}.sql"

    old_profiles = fetch_old_profiles()
    old_comp = fetch_old_competitions_2025()
    old_rs = fetch_old_round_scores()
    ...
    for wn in week_numbers:
        ...
        if not results:
            ...
            continue
        try:
            sql_blocks.append(build_sql(wn, comp, results, round_type=args.round_type))
    ...
    header = (
        f"-- Batch 2025 summer weeks: {week_numbers[0]}..{week_numbers[-1]} "
